#Importing necessary libraries and modules for the agents and the team chat setup
import asyncio
import os

# Importing agent classes, termination conditions, team chat structure, UI console, and model client
...
# Setting the OpenAI API key as an environment variable for authentication with the OpenAI services. 
# Replace the placeholder with your actual API key.
os.environ["OPENAI_API_KEY"] = "sk-XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"  # Replace with your actual OpenAI API key

# This script sets up a sequential user registration process using three agents: DatabaseAgent, APIAgent, and ExcelAgent.
# Each agent has a specific role in the process, and they communicate with each other through a round-robin group chat. 
# The process is as follows:
# 1. DatabaseAgent retrieves random registration data from a MySQL database and formats it for the next agent.
# 2. APIAgent reads the registration data, constructs API calls to register and log in the user, and reports the results.
# 3. ExcelAgent waits for APIAgent to complete, checks the results, and saves successful registration details to an Excel file.
#  The entire process is orchestrated in a way that ensures each agent completes its task before the next one begins, with clear communication and data passing between them.
async def main():
    ...
    database_agent = factory.create_database_agent( system_message=("""
            ...
            Your task:
            1. Connect to the MySQL database '<database_name>'.
